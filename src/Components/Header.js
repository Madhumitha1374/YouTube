import React, { useEffect, useState } from 'react'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faBars, faMagnifyingGlass , faBell, faCircleUser, faMoon, faSun} from '@fortawesome/free-solid-svg-icons'
import { useDispatch, useSelector } from 'react-redux'
import { toggleMode, togglemenu } from '../redux/appSlice'
import { YOUTUBE_SEARCH_API } from '../utils/constants'
import { searchCache, searchValueResult } from '../redux/searchSlice'
import { SearchResults } from './SearchResults'

export const Header = () => {
    const dispatch = useDispatch()
    const [showSearchResults, setShowSearchResults] = useState(false)
    const [searchValue, setSearchValue] = useState("")
    const [searchResults, setSearchResults] = useState([])
    const searchCacheResults = useSelector(store => store.search)
    
    const isNightMode = useSelector(store => store.app.isNightMode)
    const toggleSidebar = () => {
        dispatch(togglemenu())
    }

    const getSearchResults = async() => {
        const data = await fetch(YOUTUBE_SEARCH_API + searchValue)
        const json = await data.json()
        setSearchResults(json[1])
        dispatch(searchCache({
            [searchValue] : json[1],
        }))
    }

    useEffect(()=>{
       
        const timer = setTimeout(()=>{
            if(searchCacheResults[searchValue]){
                setSearchResults(searchCacheResults[searchValue])
            }
            else{
                getSearchResults()
                
            }},200)

        return () => {
            clearTimeout(timer)
        }
        
    },[searchValue])

    const changeOnSearchValue = (e) => {
        // console.log(e.target.value)
        setSearchValue(e.target.value)
    }


    return (
        <div className={`grid grid-flow-col px-4 py-4 ${isNightMode ? 'bg-white' : 'bg-black text-white'}`}>
            <div className='flex col-span-2'>
                <FontAwesomeIcon icon={faBars} className='h-[22px] pl-2 cursor-pointer hidden md:inline-block lg:inline-block' onClick={toggleSidebar}/>
            {/* <img
                className='h-[22px] pl-5 cursor-pointer hidden lg:inline-block'
                onClick={() => window.location.href = "/"}
                src= "data:image/png;base64,"
                alt="youtube-logo"/> 
            <img
                className='h-[22px] pl-5 cursor-pointer hidden lg:inline-block'
                onClick={() => window.location.href = "/"}
                src= "https://i.pinimg.com/736x/3a/36/20/3a36206f35352b4230d5fc9f17fcea92.jpg"
                alt="youtube-logo"/>  */}
            <div className='flex ml-7' onClick={() => window.location.href = "/"}>
            <img
                className='h-[22px] cursor-pointer hidden border border-red-500 lg:inline-block rounded-l-lg rounded-r-lg'
                
                src= "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSLTi2ub8_T_7xRaWF7ZU36P-8jSs9sP6TvGg&usqp=CAU"
                alt="youtube-logo"/> 
                <p className=' cursor-pointer text-[20px] mt-[-5px] h-8 font-bold font-[YouTube Sans Bold]'>Youtube</p>
            </div>
           
            <img
                className='h-[22px] w-[100%] md:w-[50%] pl-5 cursor-pointer inline-block lg:hidden'
                onClick={() => window.location.href = "/"}
                src= "https://cdn.pixabay.com/photo/2016/04/24/14/36/youtube-1349702_960_720.png"
                alt="youtube-logo"/> 
            
            </div>
            
            
            <div className='flex flex-col col-span-8'>
                <div className={`h-10 grid grid-flow-col border align-middle border-gray-400 w-[80%] ml-[10%] mr-[10%] rounded-full ${isNightMode ? 'bg-white' : 'bg-black text-white'}`}>
                    <div className='pt-2 px-3 col-span-11 '>   
                    <input type='text'
                    className={`w-full outline-none ${isNightMode ? 'bg-white' : 'bg-black text-white'}`}
                    placeholder='Search'
                                   value={searchValue}
                                   onFocus={() => setShowSearchResults(true)}
                                   onBlur={() => setShowSearchResults(false)}
                                   onChange={(e)=> changeOnSearchValue(e)}
                                   onKeyDown={(e) => {
                                       if(e.key === "Enter"){
                                           window.location.href = "/results?search_query=" + searchValue
                                           setSearchValue("")
                                       }
                                   }}/>
                    </div>
                    <div className={`border-l rounded-r-full text-center pt-2 bg-gray-200  border-gray-400 col-span-1 ${isNightMode ? 'bg-gray-200' : 'bg-opacity-0 text-white'}`}>
                    <FontAwesomeIcon  icon={faMagnifyingGlass} onClick={() => window.location.href = "/results?search_query=" + {searchValue}}  />
                
                    </div>
                    
                    
                        {/* <SearchResults/> */}
                    {/* {showSearchResults && <div onBlur={() => setShowSearchResults(false)} className={`fixed mx-[14%] lg:w-[480px] md:w-[480px] w-full  pl-4 pr-36 mt-10 rounded-2xl shadow ${isNightMode ? 'bg-white text-black' : 'bg-black text-white'}`}>
                       {searchResults.map((s)=><p className='py-2 cursor-pointer'
                        onFocus={() => setShowSearchResults(true)}
                        onClick={() => window.location.href("/results?search_query=" + s)}
                        ><FontAwesomeIcon icon={faMagnifyingGlass} className='pr-4'/>{s}</p>)}
                        </div>} */}
                        {/* <SearchResults/> */}
                    
                    </div>
                    <div className='fixed top-14 right- left-12 bg-white w-[45%] ml-[24%] mr-[10%] shadow-2xl rounded-2xl px-3'>
                        {showSearchResults && <div onBlur={() => setShowSearchResults(false)} className=''>
                       {searchResults.map((s)=><p className='py-2 cursor-pointer'
                        onFocus={() => setShowSearchResults(true)}
                        onBlur={() => setShowSearchResults(false)}
                        onClick={() => window.location.href ="/results?search_query=" + {s}}
                        ><FontAwesomeIcon icon={faMagnifyingGlass} className='pr-4'/>{s}</p>)}
                        </div>}
                    </div>
            </div>

            
            
            {/* {showSearchResults && <div onBlur={() => setShowSearchResults(false)} className='fixed mx-[14%] w-[540px]  pl-4 pr-36 mt-10  bg-white rounded-2xl shadow '>
                {searchResults.map((s)=><p className='py-2 hover:bg-slate-200 cursor-pointer'
                 onFocus={() => setShowSearchResults(true)}
                  onClick={() =>onClickSearchValue(s)}><FontAwesomeIcon icon={faMagnifyingGlass} className='pr-4'/>{s}</p>)}
            </div>}  */}
            <div className='col-span-2 text-end'>
                {isNightMode ? <FontAwesomeIcon icon={faMoon} className='cursor-pointer pr-7 h-[22px] md:inline hidden' onClick={()=> dispatch(toggleMode())}/> :
                <FontAwesomeIcon icon={faSun} className='cursor-pointer pr-7 h-[22px] lg:inline hidden' onClick={()=> dispatch(toggleMode())}/>}
                <FontAwesomeIcon icon={faBell} className='cursor-pointer pr-7 h-[22px] lg:inline  hidden'/>
                <FontAwesomeIcon icon={faCircleUser} className='cursor-pointer h-[22px]'/>
            </div>
        </div>
    )
}